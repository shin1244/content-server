...
- 랭킹: 강화 성공·아이템 드랍 시 Redis ZSET 갱신, 서버 기동 시 DB에서 전체 리빌드,
  `/r` 상위 20명 / `/r me` 내 주변 순위
- 클라이언트: 터미널 채팅 클라이언트, 봇 클라이언트

**다음**

- 아이템이 없는 유저는 랭킹에 아예 안 들어옴 (`GetAllPower` 가 `items` 와 INNER JOIN)
...
| `Ranking` | Redis 래퍼 (순위 ZSET + 이름 HASH), 전체 하나 (내부 커넥션 풀) |
...
**이름은 ZSET 밖에** — 순위표를 보여주려면 userId에 닉네임을 붙여야 하는데, 접속하지 않은
유저는 세션에서 찾을 수 없습니다. 그렇다고 ZSET의 member를 `"1024:홍길동"` 같은 복합 키로
만들면 조회는 한 번에 끝나지만 member 자체가 엔트리의 고유키라서, 닉을 바꾸는 순간 같은
유저가 두 줄이 됩니다. 지우려면 바뀌기 전 닉을 알아야 하고 모르면 ZSCAN으로 전부 훑어야
합니다. 그래서 순위는 `lb:power` ZSET(member = userId)에, 이름은 `user:name` HASH에
따로 둡니다. 닉을 바꿔도 `HSET user:name` 한 줄이면 이미 쌓인 순위가 통째로 새 이름으로
보입니다.

**왕복 횟수로 설계** — `Top()` 은 ZREVRANGE로 id 20개를 받고 HMGET **한 번**으로 이름 20개를
받습니다. 왕복 2회. 유저마다 HGET을 부르면 21회가 되는데, 샤드당 컨슈머 스레드가 하나뿐이라
그동안 같은 샤드의 다른 패킷이 전부 큐에서 대기합니다. 이름을 짝지으려고 for문을 한 번 더
도는 비용은 수십 나노초, 왕복 하나는 0.1~1밀리초입니다. 루프를 늘려서 왕복을 줄이는 쪽이
항상 이득입니다.
...
**순위표의 이름이 전부 `unknown` 으로 나오던 문제** — `GetAllPower()` 의 SQL에 `user_name`
컬럼을 추가하면서 결과를 담는 쪽은 `{ row[0], row[1] }` 그대로 뒀습니다. `RankEntry` 가
aggregate라 세 번째 필드가 조용히 빈 문자열로 초기화됐고, 그게 그대로 HMSET 되어 레디스에
빈 이름이 저장됐습니다. 컴파일 경고 하나 없이 끝까지 흘러갔습니다. 구조체에 필드를 늘려도
기존 `{a, b}` 코드는 그대로 컴파일된다는 걸 잊고 있었습니다. 같은 시점에 `Ranking::SetName`
을 어디서도 호출하지 않아서, 리빌드 이후 접속한 유저는 계속 이름이 비어 있었습니다.
